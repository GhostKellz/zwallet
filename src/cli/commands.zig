//! Command-line interface for Zwallet
//! Provides interactive and scriptable wallet operations
...
pub const CLI = struct {
    ...
    fn cmdSend(self: *CLI, args: [][:0]u8) !void {
        ...
        if (from) |from_addr| {
            // Create transaction
            var tx = try transaction.ProtocolFactory.createTransaction(
                self.allocator, .ghostchain, from_addr, to.?, amount
            );
